...
def dataWrapper():
    ...
    def pubData(self,msg,Course_Direction):
        msg.header.stamp = rospy.Time.now()
        msg.header.frame_id = 'Course_Direction'
        msg.Direction = Course_Direction
        return msg
def talker(coursedirection):
    rospy.init_node('CourseDirention_talker', anonymous = True)
    pub = rospy.Publisher('CourseDirection', CourseDirection_msg, quene_size = 1)
    msg = CourseDirection_msg()
    rate = rospy.Rate(10)
    datawrapper = dataWrapper()
    try:
        coursedirection_msg = datawrapper.pubData(msg, coursedirection)
        pub.publish(coursedirection_msg)
        rate.sleep()
...
if __name__ == '__main__':
    listener()
    course_state_dic = {'x': WTSTinput[0], 'y': WTSTinput[1], 'phi':WTSTinput[2]}
    C = Maker(points)
    direction = C.getdirection(course_state_dic)
    talker(direction)
